...
endtime = length(acc)*controllerRate - controllerRate;
%% Simulation %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
simCounter = 1;
controllerCounter = 1;
counter_t = 1;
...
for t = 0:dt:endtime
	...
	%path_index = path_index2;
%	path_index2 = path_index;
	v_des = T(:, path_index2)*vel(index);
	a_des = acc(index)*T(:, path_index2) + vel(index)^2*curvature(path_index2)*N(:, path_index2);
	if controllerCounter > 1
		kappa_prime = (curvature(path_index2)-curvature(path_index2-1))/0.0001;
		j_des = (jerk(index) - curvature(path_index2)^2*vel(index)^3)*T(:, path_index2) +(3*curvature(path_index2)*vel(index)*acc(index)+kappa_prime*vel(index)^2)*N(:, path_index2) + curvature(path_index2)*torsion(path_index2)*vel(index)^3*B(:, path_index2);
		% FIX THIS j_des = jerk(index)*(a_des/norm(a_des)) + acc(index)*(a_des - a_des_hist(:, controllerCounter-1))/controllerRate;
	else
		j_des = [0;0;0];
	end
...
% 	if norm(j_des) > 25
% 		j_des = j_des/norm(j_des);
% 	end
	
% 	v_des = v_des*(1 - curvature(path_index2);
% 	a_des = a_des*(1 - curvature(path_index2);
% 	j_des = j_des*(1 - curvature(path_index2);
	
% 	if controllerCounter > 1
% 		% Could take derivatives analytically
% 		T = (path(path_index2, :)' - path(path_index2-1, :)')/(1/10001)/norm((path(path_index2, :)' - path(path_index2-1, :)')/(1/10001));
% 		T_hist(:, controllerCounter) = T;
% 
% 		N = ((T - T_hist(:, controllerCounter-1))/(1/10001));%/norm((T - T_hist(controllerCounter-1))/(1/10001));
% 		N_hist(:, controllerCounter) = N;
% 
% 		B = cross(T,N/norm(N));
% 		B_hist(:, controllerCounter) = B;
% 
% 		v_des = T*vel(index);
% 		a_des = acc(index)*T + vel(index)*N;
% 		j_des = jerk(index)*(a_des/norm(a_des)) + acc(index)*(a_des - a_des_hist(:, controllerCounter-1))/controllerRate;
% 	else
% 		T = [0; 0; 1];
% 		N = [0; 0; 1];
% 		B = [0; 0; 1];
...
% 		v_des = T*vel(index);
% 		a_des = acc(index)*T + vel(index)*N;
% 		j_des = jerk(index)*(a_des/norm(a_des));
% 	end
	
	
	
	%e_r = dot((path(path_index2,:)'-r_hat),N(:, path_index2))*(N(:, path_index2)) + dot((path(path_index2,:)'-r_hat),B(:, path_index2))*(B(path_index2));
	e_r = dot((path(path_index,:)'-position),N(:, path_index))*(N(:, path_index)) + dot((path(path_index,:)'-position),B(:, path_index))*(B(path_index));
	e_v = v_des - linearVel;
	e_a = a_des - linearAcc; %dot(a_des - linearAcc, N(:, path_index))*N(:, path_index) + dot(a_des - linearAcc, B(:, path_index))*B(:, path_index); % was: a_des - linearAcc
	...
	a_com = 0*e_j + 2.65*m*e_a + 14*e_r + 4.5*e_v + m*a_des -g*m*zw + F_ext; % ka = 2.5, kv = 2.5, kr = 12 works well with the external compensation and limits: v=1, a=1, j=3 (3:19pm, 6/29/20) .ka = 1, kv = 2.18, kr = 1 worked well before external force compensation (2:58pm 6/29/20)
	u1_com = norm(a_com);
	...
	M = -3.75*e_R - 2*e_w; % -1.5, -1 worked well with the external compensation on 6/29 TODO: check the SO(3) control law condition
...
% 	if controllerCounter == 1
% 		b3_prev = [0; 0; 0];
% 		a_des = acc(index)*b3_d;
% 		a_norm = a_des/norm(a_des);
% 		j_des = jerk(index)*a_norm;
% 	else
% 		[b3_prev, ~, ~, ~] = NLGL(positionHist(:,simCounter-1)', path(1:10001, :)', L, path_index_hist(controllerCounter-1));
% 		b3_deriv = (path(path_index,:)' - path(path_index-1,:)')/controllerRate; % might want to change this to get the derivative at the current loction
% 		a_des = acc(index)*b3_d - vel(index)*b3_deriv;
% 		a_norm = a_des/norm(a_des);
% 		a_norm_hist(:, controllerCounter) = a_norm;
% 
% 		a_deriv = (a_norm - a_norm_hist(:, controllerCounter-1))/controllerRate;
% 		j_des = jerk(index)*a_norm; % + acc(index)*a_deriv;
% 	end
% 	
% 	a_des = 5*b3_d;
% 	
% 	e_v = linearVel - vel(index)*b3_d;
% 	% can add in a position term too
% 	
% 	e1 = [cos(psi_s(index)); sin(psi_s(index)); 0];
% 	
% 	b3 = (a_des -g*zw)/norm(a_des -g*zw); 
% 	b2 = cross(b3, e1)/norm(cross(b3, e1));
% 	b1 = cross(b2, b3);
% 	
% 	Rd = [b1 b2 b3];
% 		
% 	h_omega = (m/norm(m*a_des -m*g*zw)) * (j_des - (dot(b3, j_des) * b3));
% 	omega_des = [dot(-h_omega, b2); dot(h_omega, b1); dot(psi_dot_s(index) * zw, b3)]; % may want to use a different set of b's
% 
% 	b3_com = (-0*e_v + a_des -g*zw)/norm(0*e_v + a_des -g*zw);
% 	b2_com = cross(b3_com, e1)/norm(cross(b3_com, e1));
% 	b1_com = cross(b2_com, b3_com);
% 	Rd = [b1_com b2_com b3_com];
% 	
% 	e_r = veeMap(0.5*(Rd'*rotMat - rotMat'*Rd))';
% 	e_w = omega - rotMat' * Rd * omega_des;
% 	M = -5*e_r - 1*e_w;
% 	
% 	Rd_hist(:, :, controllerCounter) = Rd;
% 	b3_prev_hist(:,controllerCounter) = b3_prev;
% 	% instead of b3_d, b3_prev, use the actual position
% 	%[M, omega_d, e_R, b3_com, a_des] = orientationController(b3_d, b3_prev, acc(index), vel(index), linearVel, rotMat, 20, 20, 0, I, g, controllerRate, d, e_R);
% 	b3_com_hist(:, controllerCounter) = b3_com;
% 	rotZ_hist(:, controllerCounter) = rotMat(:,3);
% 	
% 	b2_com_hist(:, controllerCounter) = b2_com;
% 	rotY_hist(:, controllerCounter) = rotMat(:,2);
% 	
% 	b1_com_hist(:, controllerCounter) = b1_com;
% 	rotX_hist(:, controllerCounter) = rotMat(:,1);
% 	
% 	a_des_hist(:, controllerCounter) = a_des;
% 	%M = [0 0.005 0];
% 	e_R_hist(:, controllerCounter) = e_r;
% 	M_hist(:, controllerCounter) = M;
% 	%omega_d_hist(:, controllerCounter) = omega_d;
% 	
% 	% adjust this
% 	% should probably add a proportional term
% 	%v_des = b3_com * vel(index);
% 	%e_v = v_des - linearVel;
% 	%e_r = d*b3_com;
% 	
% 	u1_com = dot(-0*e_v + m*a_des - m*g*zw, rotMat*zw);% * (1/grav_scale)); %dot(0*(d)*b3_d + 0*(v_des - linearVel) + 0*(a_des - linearAcc), rotMat(:,3)) + m*(acc(index))*b3_d - dot(g*m*[0;0;1], rotMat(:,3)); %dot(0*(v_des - linearVel), rotMat(:,3)) + m*a - g*m; % %1*m - g*m; % can add in vel error
% 	u1_com_hist(controllerCounter) = u1_com;
% 		
	end
